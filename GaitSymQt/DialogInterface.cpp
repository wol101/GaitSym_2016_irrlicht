    ...
    ui(new Ui::DialogInterface)
{
    ...
    m_simulation = mainWindow->GetSimulation();
}
...
void DialogInterface::colourEnvironment()
{
    ...
    if (colour.isValid())
    {
        ...
        if (m_simulation)
        {
            m_simulation->GetEnvironment()->SetColour(m_EnvironmentColour.redF(), m_EnvironmentColour.greenF(), m_EnvironmentColour.blueF(), m_EnvironmentColour.alphaF());
            m_simulation->GetInterface()->EnvironmentColour.SetColour(m_EnvironmentColour.redF(), m_EnvironmentColour.greenF(), m_EnvironmentColour.blueF(), m_EnvironmentColour.alphaF());
            emit EmitRedrawRequired();
        }
    }
}
...
void DialogInterface::colourBody()
{
    ...
    if (colour.isValid())
    {
        ...
        if (m_simulation)
        {
            ...
            m_simulation->GetInterface()->BodyColour.SetColour(m_BodyColour.redF(), m_BodyColour.greenF(), m_BodyColour.blueF(), m_BodyColour.alphaF());
            emit EmitRedrawRequired();
        }
    }
}
...
void DialogInterface::colourJoint()
{
    ...
    if (colour.isValid())
    {
        ...
        if (m_simulation)
        {
            ...
            m_simulation->GetInterface()->JointColour.SetColour(m_JointColour.redF(), m_JointColour.greenF(), m_JointColour.blueF(), m_JointColour.alphaF());
            emit EmitRedrawRequired();
        }
    }
}
...
void DialogInterface::colourGeom()
{
    ...
    if (colour.isValid())
    {
        ...
        if (m_simulation)
        {
            ...
            m_simulation->GetInterface()->GeomColour.SetColour(m_GeomColour.redF(), m_GeomColour.greenF(), m_GeomColour.blueF(), m_GeomColour.alphaF());
            emit EmitRedrawRequired();
        }
    }
}
...
void DialogInterface::colourGeomForce()
{
    ...
    if (colour.isValid())
    {
        ...
        if (m_simulation)
        {
            std::vector<Contact *> *contactList = m_simulation->GetContactList();
            ...
            m_simulation->GetInterface()->GeomForceColour.SetColour(m_GeomColour.redF(), m_GeomColour.greenF(), m_GeomColour.blueF(), m_GeomColour.alphaF());
            emit EmitRedrawRequired();
        }
    }
}
...
void DialogInterface::colourStrap()
{
    ...
    if (colour.isValid())
    {
        ...
        if (m_simulation)
        {
            ...
            m_simulation->GetInterface()->StrapColour.SetColour(m_StrapColour.redF(), m_StrapColour.greenF(), m_StrapColour.blueF(), m_StrapColour.alphaF());
            emit EmitRedrawRequired();
        }
    }
}
...
void DialogInterface::colourStrapForce()
{
    ...
    if (colour.isValid())
    {
        ...
        if (m_simulation)
        {
            m_simulation->GetInterface()->StrapForceColour.SetColour(m_StrapForceColour.redF(), m_StrapForceColour.greenF(), m_StrapForceColour.blueF(), m_StrapForceColour.alphaF());
            std::map<std::string, Muscle *> *muscleList = m_simulation->GetMuscleList();
            std::map<std::string, Muscle *>::const_iterator muscleIter;
            for (muscleIter = muscleList->begin(); muscleIter != muscleList->end(); muscleIter++)
            {
...
void DialogInterface::colourStrapCylinder()
{
    ...
    if (colour.isValid())
    {
        ...
        if (m_simulation)
        {
            m_simulation->GetInterface()->StrapCylinderColour.SetColour(m_StrapCylinderColour.redF(), m_StrapCylinderColour.greenF(), m_StrapCylinderColour.blueF(), m_StrapCylinderColour.alphaF());
            std::map<std::string, Muscle *> *muscleList = m_simulation->GetMuscleList();
            std::map<std::string, Muscle *>::const_iterator muscleIter;
            for (muscleIter = muscleList->begin(); muscleIter != muscleList->end(); muscleIter++)
            {
                ...
                {
                    cylinderWrapStrap->SetCylinderColour(m_simulation->GetInterface()->StrapCylinderColour);
                ...
                else
                {
                    ...
                    {
                        twoCylinderWrapStrap->SetCylinderColour(m_simulation->GetInterface()->StrapCylinderColour);
...
void DialogInterface::colourReporter()
{
    ...
    if (colour.isValid())
    {
        ...
        if (m_simulation)
        {
            std::map<std::string, Reporter *> *reporterList = m_simulation->GetReporterList();
            ...
            m_simulation->GetInterface()->ReporterColour.SetColour(m_ReporterColour.redF(), m_ReporterColour.greenF(), m_ReporterColour.blueF(), m_ReporterColour.alphaF());
            emit EmitRedrawRequired();
        }
    }
}
...
void DialogInterface::colourDataTarget()
{
    ...
    if (colour.isValid())
    {
        ...
        if (m_simulation)
        {
            std::map<std::string, DataTarget *> *reporterList = m_simulation->GetDataTargetList();
            ...
            m_simulation->GetInterface()->DataTargetColour.SetColour(m_DataTargetColour.redF(), m_DataTargetColour.greenF(), m_DataTargetColour.blueF(), m_DataTargetColour.alphaF());
            emit EmitRedrawRequired();
        }
    }
}
