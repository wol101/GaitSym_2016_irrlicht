...
void SimulationWindow::initialiseScene()
{
    ...
    SetMeshColour(m_cursor3D->getMesh(), irr::video::SColor(255, 200, 200, 200));
...
void SimulationWindow::mouseMoveEvent(QMouseEvent *event)
{
    ...
    else if (event->buttons() & Qt::MidButton)
    {
        if (m_zoomFlag)
        {
            ...
            else if (m_FOV < 0.001) m_FOV = 0.001;
            updateCamera();
            renderLater();
...
MyMovieHandle SimulationWindow::getQtMovie() const
{
    return m_qtMovie;
}

void SimulationWindow::setQtMovie(const MyMovieHandle &qtMovie)
{
    m_qtMovie = qtMovie;
}

SimulationWindow::MovieFormat SimulationWindow::getMovieFormat() const
{
    return m_movieFormat;
}

void SimulationWindow::setMovieFormat(const MovieFormat &movieFormat)
{
    m_movieFormat = movieFormat;
}
...
int SimulationWindow::WriteFrame(const QString &filename)
{
    switch(m_movieFormat)
    {
    case Quicktime:
        if (m_qtMovie)
        {
            ...
            if (image) //should always be true, but you never know. ;)
            {
                ...
                image->drop();
                addImageToMovie(m_qtMovie, size.Width, size.Height, rgb);
                delete [] rgb;
            }
            return 0;
        }
    ...
    case PPM:
        if (1)
        {
            ...
            if (image) //should always be true, but you never know. ;)
            {
                ...
                QString filenameWithExtension = filename + ".ppm";
                if (!videoDriver()->writeImageToFile(image, filenameWithExtension.toUtf8().data()))
                    irrlichtDevice()->getLogger()->log(L"Failed to take screenshot.", irr::ELL_WARNING);
                image->drop();
            }
        }
        break;
    ...
    case TIFF:
        if (1)
        {
            ...
            if (image) //should always be true, but you never know. ;)
            {
                ...
                image->drop();
                QString filenameWithExtension = filename + ".tif";
                WriteTIFF(filenameWithExtension.toUtf8(), size.Width, size.Height, (unsigned char *)rgb);
                delete [] rgb;
            }
            return 0;
        }
    ...
    case APNG:
        if (1)
        {
            ...
            if (image) //should always be true, but you never know. ;)
            {
                ...
                WriteAPNG(filenameWithExtension.toUtf8(), size.Width, size.Height, (unsigned char *)rgb, false);
                delete [] rgb;
            }
            return 0;
        }
...
void SimulationWindow::WritePPM(const char *pathname, int width, int height, unsigned char *rgb)
{
    FILE *out;
    int i;
...
// write a TIFF file
void SimulationWindow::WriteTIFF(const char *pathname, int width, int height, unsigned char *rgb)
{
    TIFFWrite tiff;
    int i;
...
// write an APNG file
void SimulationWindow::WriteAPNG(const char *pathname, int width, int height, unsigned char *rgb, bool deleteExisting)
{
    APNGWriter::WriteAPNG(pathname, static_cast<quint32>(width), static_cast<quint32>(height), rgb, deleteExisting);
}


// write the scene as a series of OBJ files in a folder
void SimulationWindow::WriteOBJ(const char *pathname)
{
...
#ifdef COPY_TEXTURE_RESOURCE_FILES
    // this code looks at all the resources in case we need some
    QDirIterator it(":", QDirIterator::Subdirectories);
    QStringList resourceFiles;
    while (it.hasNext()) resourceFiles.append(it.next());
#endif
    ...
    for (irr::scene::ISceneNodeList::ConstIterator it = allSceneNodes.begin(); it != allSceneNodes.end(); it++)
    {
        ...
        if (meshSceneNode)
        {
            ...
            if (m_movieFormat == OBJNOBODIES) nameList.push_back(meshSceneNode->getName());
            continue;
        }
        ...
        if (animatedMeshSceneNode)
        {
            ...
            if (m_movieFormat == OBJNOBODIES) nameList.push_back(animatedMeshSceneNode->getName());
            continue;
        }
    }
    ...
    {
        if (m_movieFormat == OBJNOBODIES) qDebug() << nameList[meshCount];
        if (m_movieFormat == OBJNOBODIES && DataFile::StringStartsWith(nameList[meshCount], "BODY "))
        {
            QRegExp rx("BODY GraphicsFile=\"([^\"]*)\" Offset=\"(\\S*) (\\S*) (\\S*)\" Scale=\"(\\S*) (\\S*) (\\S*)\"");
            int pos = rx.indexIn(nameList[meshCount]);
            Q_UNUSED(pos);
            QStringList list = rx.capturedTexts();
            QString filename = list[1];
            irr::core::vector3df graphicsOffset(list[2].toFloat(), list[3].toFloat(), list[4].toFloat());
            irr::core::vector3df graphicScale(list[5].toFloat(), list[6].toFloat(), list[7].toFloat());
            irr::core::matrix4 preTransform;
            preTransform.setTranslation(graphicsOffset);
            preTransform.setScale(graphicScale);
            QString baseName = QFileInfo(filename).completeBaseName();
            QFile file(QString(baseName) + QString(".trans"));
            file.open(QIODevice::WriteOnly);
            irr::core::matrix4 globalTransform = *transformationList[meshCount];
            irr::core::matrix4 matrix = globalTransform * preTransform;
            irr::core::vector3df euler = matrix.getRotationDegrees();
            irr::core::vector3df scale = matrix.getScale();
            irr::core::vector3df translation = matrix.getTranslation();
            irr::core::quaternion quaternion(matrix);
            file.write(QString("# transformation file exported from GaitSym2016\n").toUtf8());
            file.write(QString("path\n\"%1\"\n").arg(filename).toUtf8());
            file.write(QString("scale\n%1 %2 %3\n").arg(scale.X,0,'g',9).arg(scale.Y,0,'g',9).arg(scale.Z,0,'g',9).toUtf8()); // %1.8e or %.9g needed not to lose accuracy for float
            file.write(QString("euler\n%1 %2 %3\n").arg(euler.X,0,'g',9).arg(euler.Y,0,'g',9).arg(euler.Z,0,'g',9).toUtf8());
            file.write(QString("quaternion\n%1 %2 %3 %4\n").arg(quaternion.W,0,'g',9).arg(quaternion.X,0,'g',9).arg(quaternion.Y,0,'g',9).arg(quaternion.Z,0,'g',9).toUtf8());
            file.write(QString("translation\n%1 %2 %3\n").arg(translation.X,0,'g',9).arg(translation.Y,0,'g',9).arg(translation.Z,0,'g',9).toUtf8());
            file.write(QString("matrix\n").toUtf8());
            file.write(QString("%1 %2 %3 %4\n").arg(matrix(0,0),0,'g',9).arg(matrix(0,1),0,'g',9).arg(matrix(0,2),0,'g',9).arg(matrix(0,3),0,'g',9).toUtf8());
            file.write(QString("%1 %2 %3 %4\n").arg(matrix(1,0),0,'g',9).arg(matrix(1,1),0,'g',9).arg(matrix(1,2),0,'g',9).arg(matrix(1,3),0,'g',9).toUtf8());
            file.write(QString("%1 %2 %3 %4\n").arg(matrix(2,0),0,'g',9).arg(matrix(2,1),0,'g',9).arg(matrix(2,2),0,'g',9).arg(matrix(2,3),0,'g',9).toUtf8());
            file.write(QString("%1 %2 %3 %4\n").arg(matrix(3,0),0,'g',9).arg(matrix(3,1),0,'g',9).arg(matrix(3,2),0,'g',9).arg(matrix(3,3),0,'g',9).toUtf8());
            file.close();
        }
        else
        {
...
#ifdef COPY_TEXTURE_RESOURCE_FILES
                    for (int resourceIndex = 0; resourceIndex < resourceFiles.size(); resourceIndex++)
                    {
                        if (resourceFiles.at(resourceIndex).endsWith(textureBasename.c_str()))
                        {
                            QImage qImage;
                            qImage.load(resourceFiles.at(resourceIndex));
                            qImage.save(QString(textureBasename.c_str()));
                        }
                    }
#endif
                }
...
void SimulationWindow::SetMeshColour(irr::scene::IMesh *mesh, const irr::video::SColor &colour)
{
    ...
    float ambientProportion = 0.2;
    float diffuseProportion = 0.8;
    float specularProportion = 0.3;
    float emissiveProportion = 0.0;
    float shininess = 20;
