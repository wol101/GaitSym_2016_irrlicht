...
void FacetedObject::initializeSceneNode()
{
    ...
    meshbuffer->getVertexBuffer().reallocate(GetNumVertices());
    meshbuffer->getIndexBuffer().reallocate(GetNumTriangles() * 3);
    meshbuffer->setHardwareMappingHint(irr::scene::EHM_STATIC);
    ...
    for (i = 0; i < GetNumVertices(); i++)
    {
        ...
        // v.TCoords.set(tu, tv);
        meshbuffer->getVertexBuffer().push_back(v);
    ...
    {
        meshbuffer->getIndexBuffer().push_back(*indexList++);
        meshbuffer->getIndexBuffer().push_back(*indexList++);
        meshbuffer->getIndexBuffer().push_back(*indexList++);
    }

    meshbuffer->setBoundingBox(irr::core::aabbox3df(minx, miny, minz, maxx, maxy, maxz));
    m_simulationWindow->sceneManager()->getMeshManipulator()->recalculateNormals(meshbuffer);

    irr::scene::SMesh *mesh = new irr::scene::SMesh();
    mesh->addMeshBuffer(meshbuffer);
    mesh->recalculateBoundingBox();
    m_simulationWindow->SetMeshColour(mesh, vertexColour);
    meshbuffer->drop();
    ...
    irr::s32 idBitMask = 1 << 0;
    m_sceneNode = m_simulationWindow->sceneManager()->addAnimatedMeshSceneNode(animatedMesh, 0, idBitMask);
    animatedMesh->drop();
    int minimalPolysPerNode = 256;
