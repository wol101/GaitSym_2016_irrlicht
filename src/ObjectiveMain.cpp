...
#ifdef USE_CARBON
int carbonMain(int argc, char* argv[]);
#endif
...
static bool gModelStateRelative = true;
...
int gAxisFlag = true;
int m_drawMuscleForces = false;
int gDrawContactForces = false;
int gDrawMuscles = true;
int gDrawBonesFlag = true;
int gDrawGeomsFlag = true;
int gWhiteBackground = false;
int m_activationDisplay = true;
int gElasticDisplay = false;
...
#endif
{
#if defined(USE_QT) && !defined(USE_IRRLICHT) && !defined(USE_CARBON)
    ...
    if (gModelConfigFile)
    {
        ...
        if (gDebug == MainDebug)
        {
            *gDebugStream <<  "main loaded gModelConfigFile " << gModelConfigFile << "\n";
        }
    }
    ...
    return carbonMain(argc, argv);
#else
#if defined(USE_IRRLICHT)
    // Jump to the Irrlicht code (returns on exit)
    return StartIrrlicht();
#else
...
void ParseArguments(int argc, char ** argv)
{
    ...
    for (i = 0; i < argc; i++)
    {
            ...
            strcmp(argv[i], "-s") == 0)
        {
            ...
            if (strcmp(argv[i], "--ModelStateWorld") == 0 ||
                strcmp(argv[i], "-e") == 0)
            {
                gModelStateRelative = false;
            }
        else
...
int ReadModel(void *userData)
{
    ...
    if (gConfigFilenamePtr)
    {
        ...
        if (gDebug == MainDebug) *gDebugStream <<  "ReadModel (MPI) read file " << gConfigFilenamePtr << "\n";
    }
    else
    {
        ...
        if (gDebug == MainDebug) std::cerr << "ReadModel MPI_Probe status.MPI_TAG " << status.MPI_TAG << "\n";

        int count;
        MPI_Get_count(&status, MPI_BYTE, &count);
        if (gDebug == MainDebug) std::cerr << "ReadModel MPI_Get_count count " << count << "\n";
        ...
        int *iPtr = (int *)data;
        if (gDebug == MainDebug) std::cerr << "ReadModel MPI_Recv MPI_MESSAGE_ID " << iPtr[0] << "\n";
        if (iPtr[0] == MPI_MESSAGE_ID_RELOAD_MODELCONFIG) // force a model reload
        {
            gXMLConverter.LoadBaseXMLString(data + 2 * sizeof(int));
            if (gDebug == MainDebug)
            {
                *gDebugStream <<  "ReadModel loaded MPI MODELCONFIG\n";
            }
            delete [] data;
            return 1;
        }
        ...
        else if (iPtr[0] == MPI_MESSAGE_ID_SEND_TIMINGS)
        {
            ...
            dPtr[1] = gIOTime;
            if (gDebug == MainDebug) std::cerr << "ReadModel MPI_Send timings " << results[0] << "\n";
    ...
    if (gSimulation->LoadModel(myFile.GetRawData()))
    {
        delete gSimulation;
        gSimulation = 0;
        return 1;
...
int WriteModel()
{
    ...
    iPtr[1] = gRunID;
    dPtr[0] = score;
    if (gDebug == MainDebug) std::cerr << "WriteModel MPI_Send results[0] " << results[0] << "\n";
    MPI_Send(results,            /* message buffer */
             sizeof(results),    /* 'len' data item */
             MPI_BYTE,           /* data items are bytes */
             0,                  /* destination */
