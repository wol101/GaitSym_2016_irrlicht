...
#ifdef USE_QT
                if ((!strcmp(cur->name(), "INTERFACE"))) ParseInterface(cur);
#endif
            }
...
void Simulation::ParseDriver(rapidxml::xml_node<char> * cur)
{
    ...
    if (strcmp((const char *)buf, "Cyclic") == 0)
    {
        ...
        buf = DoXmlGetProp(cur, "Target");
        if (buf == 0) THROWIFZERO(buf = DoXmlGetProp(cur, "TargetID"));
...
void Simulation::ParseDataTarget(rapidxml::xml_node<char> * cur)
{
    ...
    if (strcmp((const char *)buf, "Scalar") == 0)
    {
        ...
        buf = DoXmlGetProp(cur, "DurationValuePairs");
        if (buf)
        {
...
#ifdef USE_QT
void Simulation::ParseInterface(rapidxml::xml_node<char> * cur)
{
    char  *buf;

    buf = DoXmlGetProp(cur, "TrackBodyID");
    if (buf)
    {
...
void Simulation::OutputProgramState()
{
    ...
    for (currentNode = currentNode->first_node(); currentNode; currentNode = currentNode->next_sibling())
    {
        if (
                ...
                strcmp(currentNode->name(), "DRIVER") == 0 ||
