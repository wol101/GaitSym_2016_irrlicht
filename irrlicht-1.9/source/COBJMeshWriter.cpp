...
bool COBJMeshWriter::writeMesh(io::IWriteFile* file, scene::IMesh* mesh, s32 flags)
{
    ...
    for (u32 i=0; i<mesh->getMeshBufferCount(); ++i)
    {
        ...
        if (buffer && buffer->getVertexCount())
        {
            ...
            // wis - altered to allow video::EIT_32BIT indices
            // wis - also altered the winding direction on the triangles
            if (buffer->getIndexType() == video::EIT_16BIT)
            {
                ...
                {
                    file->write("f ",2);
                    // num = core::stringc(buffer->getIndices()[j+2]+allVertexCount); // wis
                    ...
                    // num = core::stringc(buffer->getIndices()[j+0]+allVertexCount); // wis
                ...
                // wis - altered to allow EIT_32BIT indices
            }
            else
            {
                for (j=0; j<indexCount; j+=3)
                {
                    file->write("f ",2);
                    // num = core::stringc(reinterpret_cast<u32 *>(buffer->getIndices())[j+2]+allVertexCount); // wis
                    num = core::stringc(reinterpret_cast<u32 *>(buffer->getIndices())[j+0]+allVertexCount);
                    file->write(num.c_str(), num.size());
                    file->write("/",1);
                    file->write(num.c_str(), num.size());
                    file->write("/",1);
                    ...
                    num = core::stringc(reinterpret_cast<u32 *>(buffer->getIndices())[j+1]+allVertexCount);
                    file->write(num.c_str(), num.size());
                    file->write("/",1);
                    file->write(num.c_str(), num.size());
                    file->write("/",1);
                    ...
                    // num = core::stringc(reinterpret_cast<u32 *>(buffer->getIndices())[j+0]+allVertexCount); // wis
                    num = core::stringc(reinterpret_cast<u32 *>(buffer->getIndices())[j+2]+allVertexCount);
                    file->write(num.c_str(), num.size());
                    file->write("/",1);
                    file->write(num.c_str(), num.size());
                    file->write("/",1);
...
{
    // wis - the original code alters the handedness of the data and I don't want this
    //s = core::stringc(-v.X);
    s = core::stringc(v.X);
    // ~ wis
    s += " ";
